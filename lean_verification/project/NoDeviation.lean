...
* `nodeviation_perfect`, for the sub-case `ν(F) = n - 2` of (C-iii), where the crude bound is not
  enough and Lemma 1 is used instead: O's unique completing cell is X's own last Phase-1 stone
...
/-- **Lemma 12**, cases (C-i) and (C-iii) with `ν(F) ≤ n-3` of `main.tex`: if O's set is
...
/-- **Lemma 12**, case (C-ii) of `main.tex`: the mirror statement of `nodeviation_col`,
...
/-- **Lemma 12**, case (C-iii) with `ν(F) = n - 2`, of `main.tex`. Here `F` is a perfect
matching of `A × B` (Lemma 8), `e = u_a` is X's last Phase-1 row and `σ e = v_c`. O's set at
