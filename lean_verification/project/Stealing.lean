...
/-!
# Player 2 never wins (§2 of `main.tex`)
...
/-- **Strategy stealing** (§2 of `main.tex`), the inductive step. `p` is the real position
...
/-- **Strategy stealing** (§2 of `main.tex`): if Player 2 could force a win on the `n × n`
...
/-- **Player 2 never wins** (§2 of `main.tex`): for every `n ≥ 2` Player 2 has no strategy
